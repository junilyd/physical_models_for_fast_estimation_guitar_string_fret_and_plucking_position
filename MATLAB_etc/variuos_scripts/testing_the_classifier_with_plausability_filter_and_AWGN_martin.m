...
fileName = strcat(sprintf('trained_model_of_martin_from_%1.0fth_fret',trainingFret));
load(fileName)
...
for trueFret = testFrets
    ...
    [stringEstimateSim(trueString,fretNdx,recDir), fretEstimateSim(trueString,fretNdx,recDir)] = ...
        evaluation_of_simulated_classifier_on_acoustic_function([pitchEstimate(trueString,fretNdx,recDir), BEstimate(trueString,fretNdx,recDir)]');
