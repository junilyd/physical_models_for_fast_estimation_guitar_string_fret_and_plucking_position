function r = mirfeatures(extr,x,varargin)
...
if isa(x,'miraudio') || isa(x,'mirdesign')
    a = miraudio(x,'Normal','Label',8:8,'Extract',0,extr) % normalize with respect to RMS energy 
                              % in order to consider timbre independently of
                             % energy
else
    a = miraudio('Design','Normal','Label',8:8,'Extract',0,extr);
end
